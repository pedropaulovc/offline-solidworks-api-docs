...
Usage:
    uv run python 03_crawl_type_members/run_crawler.py [options]
...
def check_prerequisites(metadata_dir: Path) -> bool:
    ...
    api_members_xml = metadata_dir.parent.parent / "02_extract_types" / "metadata" / "api_members.xml"
    if not api_members_xml.exists():
        print("\n[ERROR] Phase 2 output not found!")
        print(f"Expected: {api_members_xml}")
        print("Please run Phase 2 (02_extract_types) first:")
        print("  uv run python 02_extract_types/extract_members.py")
        return False
...
def main() -> None:
    ...
    try:
        ...
        print("You can resume later with: uv run python 03_crawl_type_members/run_crawler.py --resume")
        sys.exit(1)
