...
- ✅ Phase 1 complete (01_crawl_toc_pages)
- ✅ Phase 2 complete (02_extract_types)
...
uv run python 03_crawl_type_members/run_crawler.py --test

# Resume from previous crawl
uv run python 03_crawl_type_members/run_crawler.py --resume

# Crawl with validation
uv run python 03_crawl_type_members/run_crawler.py --validate
```
...
```bash
# Basic validation
uv run python 03_crawl_type_members/validate_crawl.py
...
```bash
# Run all tests
uv run pytest 03_crawl_type_members/tests/ -v

# Run with coverage
uv run pytest 03_crawl_type_members/tests/ --cov=03_crawl_type_members --cov-report=html
```
...
def load_urls(self):
    ...
    xml_file = Path(...) / "02_extract_types" / "metadata" / "api_members.xml"
...
```bash
uv run python 02_extract_types/extract_members.py
```
