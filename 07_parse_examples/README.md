...
Parses HTML example files from Phase 05 and generates XML with CDATA-wrapped example content.

## Overview

Phase 06 takes the HTML example files crawled in Phase 05 and extracts the example content (descriptions, code snippets, etc.) into a structured XML format. The XML contains `<Example>` elements with URLs and CDATA-wrapped content including code blocks.

## Prerequisites

- **Phase 05** must be completed (HTML files in `05_crawl_examples/output/html/`)
- Python 3.12+
...
```bash
uv run python 06_parse_examples/parse_examples.py
```

This will:
1. Read all HTML files from `05_crawl_examples/output/html/`
...
```bash
uv run python 06_parse_examples/validate_parse.py
```

Add `--verbose` to see all warnings:

```bash
uv run python 06_parse_examples/validate_parse.py --verbose
```

### Run Tests

```bash
uv run pytest 06_parse_examples/tests/ -v
```
...
- **Relative URLs**: Preserved from Phase 05 directory structure
...
```json
{
  ...
  "input_directory": "05_crawl_examples/output/html",
  "output_file": "06_parse_examples/output/examples.xml",
  "total_examples": 1198
}
```
...
- ✅ 100% coverage against Phase 05 HTML files
...
- Check if Phase 05 completed successfully
...
- HTML files: `05_crawl_examples/output/html/**/*.htm`
- Used for: Example content source

### Outputs for Future Phases

- `output/examples.xml`: Structured example data for:
  - Phase 07+: Building searchable documentation
  - Phase 08+: Generating IntelliSense XML
