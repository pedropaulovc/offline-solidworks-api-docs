#!/usr/bin/env python
"""
Main entry point for the SolidWorks API documentation crawler.
...
def validate_crawl(metadata_dir):
    ...
    # Check for minimum page count (expecting at least 450 for full crawl)
    if not test_mode and crawled_count < 450:
        print(f"\n[WARNING]  WARNING: Only {crawled_count} pages crawled (expected >= 450)")
        return False
